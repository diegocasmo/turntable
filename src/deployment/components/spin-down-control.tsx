  ...
  return (
    <div className="grid justify-items-end gap-2">
      <AlertDialog.Root onOpenChange={(open) => open && spinDown.reset()}>
        ...
        <AlertDialog.Portal>
          <AlertDialog.Backdrop className="fixed inset-0 z-40 min-h-dvh bg-[var(--shadow-color)]/80 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0" />
          <AlertDialog.Popup className="fixed top-1/2 left-1/2 z-50 grid w-[min(28rem,calc(100vw-3rem))] -translate-x-1/2 -translate-y-1/2 gap-6 border border-border bg-card p-6 text-foreground shadow-[10px_10px_0_var(--shadow-color)] transition-[scale,opacity] duration-100 data-ending-style:scale-[0.98] data-ending-style:opacity-0 data-starting-style:scale-[0.98] data-starting-style:opacity-0">
            <div className="grid gap-2">
              <AlertDialog.Title className="text-xl font-medium">
                Spin down deployment?
              </AlertDialog.Title>
              <AlertDialog.Description className="text-sm leading-6 text-foreground-soft">
      ...
      {spinDown.error ? (
        <p
          id="spin-down-error"
          role="alert"
          className="max-w-72 text-right text-sm text-danger-foreground"
        >
