...
5. Do not use subscriptions or general polling in application code. The Services collection query can refetch every 5 seconds while at least one shown deployment has a transitional status, or while an action invalidation has not recovered. It must stop after stable data or route unmount. The real Railway end-to-end cleanup can also poll at 1, 2, 4, and then 5 second intervals for at most two minutes.
