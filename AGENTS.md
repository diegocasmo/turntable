...
7. Keep each pull request at or below the added-line limit that [the Dangerfile](./dangerfile.ts) defines and enforces. The check counts tests like other files. Split the work instead.
...
19. In component tests, query the rendered user interface by semantic role and accessible name. Do not assert serialized HTML with substring matches.
