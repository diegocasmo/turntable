...
5. Do not use subscriptions or general polling in application code. The Services collection query can refetch every 5 seconds while an accepted operation is not visible or at least one shown deployment has a transitional status. It must stop after the operation appears, all shown data is stable, or the route unmounts. The real Railway end-to-end cleanup can also poll at 1, 2, 4, and then 5 second intervals for at most two minutes.
...
29. When a component test helper only varies component props, accept typed partial props and apply them after complete defaults. Use explicit typed options for router, provider, mock, or controlled-state harnesses.
